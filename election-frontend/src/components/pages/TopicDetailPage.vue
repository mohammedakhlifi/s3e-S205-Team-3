    ...
    async fetchPost() {
      try {
        const response = await axios.get(
            `https://election-backend-i358.onrender.com/api/forum/topics/${this.$route.params.id}`
        );
    ...
    async submitReply() {
      ...
      try {
        const response = await axios.post(
            `https://election-backend-i358.onrender.com/api/forum/topics/${this.$route.params.id}/replies`,
