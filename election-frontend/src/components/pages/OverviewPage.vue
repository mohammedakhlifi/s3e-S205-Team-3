...
export default {
  ...
  methods: {
    async fetchTopics(page = 0) {
      try {
        const response = await axios.get("https://election-backend-i358.onrender.com/api/forum/topics", {
          params: { page, size: 5 },
        });
    ...
    async submitReply(topicId) {
      try {
        ...
        const response = await axios.post(`https://election-backend-i358.onrender.com/api/forum/topics/${topicId}/replies`, {
